...
impl Value {
    ...
    pub fn object_by(&self, keys: &[&str]) -> Result<&Self> {
        self.object_maybe_by(keys)?.ok_or_else(|| Error::from(__!("There is no value at: {:?}", keys)))
    }
    ...
    pub fn object_mut_by(&mut self, keys: &[&str]) -> Result<&mut Self> {
        self.object_maybe_mut_by(keys)?.ok_or_else(|| Error::from(__!("There is no value at: {:?}", keys)))
    }
    ...
    pub fn object_take_by(&mut self, keys: &[&str]) -> Result<Self> {
        self.object_maybe_take_by(keys)?.ok_or_else(|| Error::from(__!("There is no value at: {:?}", keys)))
    }
